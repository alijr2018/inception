...
No ready-made image is pulled from Docker Hub. The only image downloaded is `debian:bookworm`,
which the subject explicitly allows; everything installed on top of it is done by the project's
own Dockerfiles.

The only way into the infrastructure is NGINX on port 443, restricted to TLSv1.2 and TLSv1.3.
The stack serves a WordPress site at `https://abrami.42.fr`.
